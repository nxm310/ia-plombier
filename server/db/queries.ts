...
export async function getOrCreateContact(phoneNumber: string, name?: string): Promise<Contact> {
  ...
  if (existing.rows.length > 0) {
    const contact = existing.rows[0] as unknown as Contact;
    ...
    return {
      ...contact,
      tags: typeof (contact as any).tags === 'string' ? JSON.parse((contact as any).tags || '[]') : ((contact as any).tags || [])
    };
  }
...
export async function updateContact(id: number, fields: Partial<Contact>): Promise<void> {
  const now = new Date().toISOString();
  ...
  if (fields.email !== undefined) { updates.push('email = ?'); args.push(fields.email); }
  if (fields.company !== undefined) { updates.push('company = ?'); args.push(fields.company); }
  if (fields.status !== undefined) { updates.push('status = ?'); args.push(fields.status); }
  if (fields.tags !== undefined) { updates.push('tags = ?'); args.push(JSON.stringify(fields.tags)); }
  if (fields.notes !== undefined) { updates.push('notes = ?'); args.push(fields.notes); }
  if (fields.ai_enabled !== undefined) { updates.push('ai_enabled = ?'); args.push(fields.ai_enabled ? 1 : 0); }
  if (fields.avatar !== undefined) { updates.push('avatar = ?'); args.push(fields.avatar); }
...
export async function getMessagesByContact(contactId: number, limit = 50): Promise<Message[]> {
  const res = await db.execute({
    sql: `SELECT * FROM (
...
export async function getMessageByWhatsAppId(whatsappMessageId: string): Promise<Message | null> {
  const res = await db.execute({
    sql: 'SELECT * FROM messages WHERE whatsapp_message_id = ?',
    args: [whatsappMessageId]
  });
  return (res.rows[0] as unknown as Message) || null;
}

// Mémoire Client
export async function getMemoriesByContact(contactId: number): Promise<Memory[]> {
  const res = await db.execute({
    sql: 'SELECT * FROM memories WHERE contact_id = ? ORDER BY created_at ASC',
    args: [contactId]
  });
  return res.rows as unknown as Memory[];
}

export async function saveOrUpdateMemory(
  contactId: number,
  category: string,
  key: string,
  value: string
...
// Collaborateurs PME
export async function getAllTeamMembers(onlyActive = false): Promise<TeamMember[]> {
  ...
  return res.rows.map(r => ({
    ...(r as any),
    status: (r as any).status || ((r as any).is_active ? 'active' : 'other'),
...
export async function updateTeamMember(id: number, data: Partial<TeamMember>): Promise<void> {
  ...
  if (data.avatar !== undefined) { updates.push('avatar = ?'); args.push(data.avatar); }
  if (data.specialties !== undefined) { updates.push('specialties = ?'); args.push(JSON.stringify(data.specialties)); }
  if (data.working_hours !== undefined) { updates.push('working_hours = ?'); args.push(JSON.stringify(data.working_hours)); }
...
export async function getAllServices(onlyActive = true): Promise<Service[]> {
  ...
  const res = await db.execute(query);
  return res.rows as unknown as Service[];
}
...
export async function getServiceById(id: number): Promise<Service | null> {
  ...
  return (res.rows[0] as unknown as Service) || null;
}
...
export async function updateService(id: number, data: Partial<Service>): Promise<void> {
  ...
  if (data.duration_minutes !== undefined) { updates.push('duration_minutes = ?'); args.push(data.duration_minutes); }
...
export async function deleteAllServices(): Promise<void> {
 ...
 */
export async function seedPlumberServices(): Promise<void> {
...
export async function getAppointments(filters?: {
  ...
  return res.rows as unknown as Appointment[];
}
...
export async function getAppointmentById(id: number): Promise<Appointment | null> {
  ...
  return (res.rows[0] as unknown as Appointment) || null;
}
...
export async function updateAppointment(id: number, data: Partial<Appointment>): Promise<void> {
  const now = new Date().toISOString();
  const updates: string[] = ['updated_at = ?'];
  const args: any[] = [now];

  if (data.title !== undefined) { updates.push('title = ?'); args.push(data.title); }
  if (data.team_member_id !== undefined) { updates.push('team_member_id = ?'); args.push(data.team_member_id); }
  if (data.service_id !== undefined) { updates.push('service_id = ?'); args.push(data.service_id); }
  ...
  if (data.document_url !== undefined) { updates.push('document_url = ?'); args.push(data.document_url); }
  if (data.document_name !== undefined) { updates.push('document_name = ?'); args.push(data.document_name); }
  if (data.collaborator_status !== undefined) { updates.push('collaborator_status = ?'); args.push(data.collaborator_status); }
  if (data.collaborator_accepted_at !== undefined) { updates.push('collaborator_accepted_at = ?'); args.push(data.collaborator_accepted_at); }
...
export async function getSetting<T = any>(key: string): Promise<T | null> {
  ...
  if (res.rows.length === 0) return null;
  try {
    return JSON.parse(res.rows[0].value as string);
  } catch {
...
export async function getCopilotMessages(limit = 100): Promise<CopilotMessage[]> {
  const res = await db.execute({
    sql: `SELECT * FROM copilot_messages ORDER BY id ASC LIMIT ?`,
    args: [limit]
  });
  return res.rows as unknown as CopilotMessage[];
}
...
export async function clearCopilotMessages(): Promise<void> {
