...
export async function chatWithCopilot(options: CopilotChatOptions): Promise<CopilotChatResponse> {
  ...
  const teamSummary = teamMembers.map(m =>
    `- ID ${m.id} : ${m.name} (${m.role}) | Statut : ${m.status || 'actif'} | Tél : ${m.phone || 'Non renseigné'}`
  ).join('\n');

  // Historique récent
  const recentHistory = await getCopilotMessages(10);
  const conversationContext = recentHistory.map(m =>
    `${m.role === 'user' ? 'Gérant' : 'Clara'} : ${m.content}`
  ).join('\n');
  ...
  async function executeCopilotTool(toolName: string, args: any): Promise<any> {
    ...
    if (toolName === 'getFinancialForecast') {
      ...
      let confirmed = 0;
      let pending = 0;
      const breakdownByService: Record<string, { count: number; total: number }> = {};
      const breakdownByMember: Record<string, { count: number; total: number }> = {};

      for (const a of apts) {
        if (a.status === 'cancelled') continue;
        const price = Number(a.service_price || 0);
        total += price;
        if (a.status === 'confirmed') confirmed += price;
        else pending += price;

        const sName = a.service_name || 'Intervention générale';
        if (!breakdownByService[sName]) breakdownByService[sName] = { count: 0, total: 0 };
        breakdownByService[sName].count++;
        breakdownByService[sName].total += price;

        const mName = a.team_member_name || 'Équipe';
        if (!breakdownByMember[mName]) breakdownByMember[mName] = { count: 0, total: 0 };
  ...
  if (geminiApiKey) {
    try {
      ...
      const model = genAI.getGenerativeModel({
        ...
        tools: [{ functionDeclarations: COPILOT_TOOLS as any }]
      });
