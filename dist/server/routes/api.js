...
apiRouter.get('/contacts/:id/messages', async (req, res) => {
    try {
...
apiRouter.post('/messages/send-file', async (req, res) => {
    try {
...
apiRouter.post('/upload-document', async (req, res) => {
    try {
...
// Mémoire Client
apiRouter.get('/contacts/:id/memories', async (req, res) => {
    try {
...
apiRouter.post('/contacts/:id/memories', async (req, res) => {
    try {
...
apiRouter.get('/team/:id', async (req, res) => {
    try {
...
apiRouter.patch('/team/:id', async (req, res) => {
    try {
        const id = Number(req.params.id);
...
apiRouter.get('/team/:id/slots', async (req, res) => {
    try {
...
apiRouter.post('/services', async (req, res) => {
    try {
        ...
        if (!name)
            return res.status(400).json({ error: 'Le nom de l\'intervention est requis' });
...
apiRouter.post('/services/seed-plumber', async (_req, res) => {
    try {
...
apiRouter.post('/services/clear-all', async (_req, res) => {
    try {
...
apiRouter.get('/presets', (_req, res) => {
    try {
...
apiRouter.get('/presets/:id', (req, res) => {
    ...
    if (!preset)
        return res.status(404).json({ error: 'Modèle sectoriel introuvable' });
    res.json(preset);
});
// Fonction générique d'application d'un preset
async function executeApplyPreset(preset, replaceServices = true) {
...
apiRouter.post('/presets/generate-custom', async (req, res) => {
    try {
...
apiRouter.post('/settings/apply-plumber-preset', async (_req, res) => {
    try {
...
apiRouter.patch('/appointments/:id', async (req, res) => {
    try {
        ...
        const shouldNotifyClient = req.body.notify_client === true ||
        ...
        const shouldNotifyCollaborator = req.body.notify_collaborator === true ||
...
apiRouter.post('/appointments/:id/collaborator-accept', async (req, res) => {
    try {
        const id = Number(req.params.id);
...
apiRouter.post('/appointments/:id/collaborator-decline', async (req, res) => {
    try {
        const id = Number(req.params.id);
        await updateAppointment(id, {
...
apiRouter.get('/appointments/:id/google', async (req, res) => {
    try {
        const id = Number(req.params.id);
...
apiRouter.get('/appointments/:id/apple', async (req, res) => {
    try {
        const id = Number(req.params.id);
        const apt = await getAppointmentById(id);
        if (!apt)
...
apiRouter.get('/appointments/:id/ics', async (req, res) => {
    try {
        const id = Number(req.params.id);
        const apt = await getAppointmentById(id);
        if (!apt)
...
apiRouter.get('/appointments/:id/calendar', async (req, res) => {
    try {
        const id = Number(req.params.id);
        const apt = await getAppointmentById(id);
        if (!apt)
...
apiRouter.get('/appointments/:id/mission', async (req, res) => {
    try {
        const id = Number(req.params.id);
        const apt = await getAppointmentById(id);
        if (!apt)
    ...
    <div class="p-5 sm:p-6 space-y-5">
      ...
      <div class="bg-slate-50 border border-slate-200/70 rounded-2xl p-4 space-y-3">
        ...
        <div>
          ...
          ${contact?.address ? `<p class="text-xs text-slate-500 mt-1">📍 ${contact.address}</p>` : ''}
        </div>
...
apiRouter.post('/settings/:key', async (req, res) => {
    try {
        const key = req.params.key;
