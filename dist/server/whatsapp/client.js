...
import { getOrCreateContact, saveMessage, getContactById, getMessageByWhatsAppId, normalizePhone } from '../db/queries.js';
...
const msgRetryCounterCache = {
    ...
    set: (key, value) => {
        ...
        if (msgRetryCounterMap.size > 1000) {
            ...
            if (firstKey)
                msgRetryCounterMap.delete(firstKey);
        }
    },
    del: (key) => {
...
export function setWhatsAppBroadcast(cb) {
...
export async function initWhatsAppClient(forceReset = false) {
    if (forceReset) {
    ...
    try {
        ...
        async function processMessageMedia(rawMsg, sockInstance, loggerInstance) {
            const messageContent = rawMsg.message;
            if (!messageContent)
                return { text: '' };
            const msg = messageContent.ephemeralMessage?.message ||
            ...
            if (msg.audioMessage) {
                ...
                try {
                    if (sockInstance) {
                        audioBuffer = (await downloadMediaMessage(rawMsg, 'buffer', {}, {
            ...
            for (const msg of m.messages) {
                ...
                if (msg.key.fromMe) {
                    ...
                    try {
                        ...
                        const savedOutbound = await saveMessage(contact.id, 'outbound', 'human', messageText, msg.key.id || undefined, 'sent', mediaInfo.mediaType || null, mediaInfo.mediaUrl || null, mediaInfo.fileName || null, mediaInfo.fileSize || null, mediaInfo.transcription || null);
                ...
                try {
                    ...
                    const savedInbound = await saveMessage(contact.id, 'inbound', 'client', messageText, msg.key.id || undefined, 'received', mediaInfo.mediaType || null, mediaInfo.mediaUrl || null, mediaInfo.fileName || null, mediaInfo.fileSize || null, mediaInfo.transcription || null);
                    broadcast('new_message', {
                        contact,
                    ...
                    if (currentContact && currentContact.ai_enabled === 1) {
                        ...
                        if (aiReply && aiReply.trim()) {
                            ...
                            const savedOutbound = await saveMessage(contact.id, 'outbound', 'ai', aiReply, sent?.key?.id || undefined, 'sent');
                            broadcast('new_message', {
                                contact: currentContact,
                                message: savedOutbound
                            });
 ...
 */
export async function sendManualWhatsAppMessage(phoneNumber, text, explicitContactId) {
    ...
    const savedMsg = await saveMessage(contact.id, 'outbound', 'human', text, sent?.key?.id || undefined, 'sent');
    broadcast('new_message', {
        contact,
        message: savedMsg
    });
 ...
 */
export async function sendManualWhatsAppFile(phoneNumber, filePath, originalFileName, mimeType, caption) {
    if (!sock || state.status !== 'connected') {
