...
import { getMemoriesByContact, getMessagesByContact, getSetting, getAllTeamMembers, getAllServices, getAppointments } from '../db/queries.js';
...
export async function generateAgentReply(options) {
    ...
    const teamContext = teamMembers.map(m => `- ${m.name} (${m.role}) : Spécialités: ${m.specialties.join(', ')}`).join('\n');
    // Récupérer les services
    const services = await getAllServices(true);
    const servicesContext = services.map(s => `- ${s.name} (durée: ${s.duration_minutes} min, tarif: ${s.price > 0 ? s.price + '€' : 'Gratuit'})`).join('\n');
...
async function runOpenAIAgent(params) {
    ...
    const openAiTools = AI_TOOLS_DECLARATIONS.map(t => ({
        type: 'function',
        function: {
            name: t.name,
            description: t.description,
 ...
 */
function fallbackResponse(text, contact, company) {
